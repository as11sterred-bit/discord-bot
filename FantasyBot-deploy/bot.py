...
try:
    ...
    # На хостинге переменные уже передаются через окружение контейнера.
    # Локальный .env остаётся удобным, но не является обязательным.
    def load_dotenv():
        return False
...
class FantasyBot(commands.Bot):
    async def setup_hook(self):
        print("Начинаю регистрацию команд Discord...", flush=True)
        ...
        print(
            f"Загружено команд: {len(synced_commands)} ({command_names})",
            flush=True,
        )
...
async def on_interaction(interaction: discord.Interaction):
    ...
    print(
        "Получено взаимодействие: "
        f"тип={interaction.type}, команда={command_name}, "
        f"сервер={interaction.guild_id}",
        flush=True,
    )
...
@bot.tree.command(
    name="ping",
    description="Проверить связь с FantasyBot",
)
@app_commands.guild_only()
async def ping(interaction: discord.Interaction):
    print("Выполняю /ping", flush=True)
    await interaction.response.send_message(
        "✅ FantasyBot на связи и получает команды.",
        ephemeral=True,
    )
...
async def panel(interaction: discord.Interaction):
    print("Выполняю /panel", flush=True)

    # Сразу подтверждаем команду — до подготовки картинки и сообщения.
...
async def ticket_panel(interaction: discord.Interaction):
    print("Выполняю /ticket-panel", flush=True)
...
async def panel_error(
    ...
    print(f"Ошибка /panel: {error!r}", flush=True)
...
async def ticket_panel_error(
    ...
    print(f"Ошибка /ticket-panel: {error!r}", flush=True)
...
print("Запускаю FantasyBot...", flush=True)
bot.run(TOKEN)
